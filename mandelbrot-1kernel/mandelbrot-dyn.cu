...
(int* d_ns, int* d_offs, int* dwells, int w, int h, complex cmin, complex cmax, int d, int depth) {
	// check whether all boundary pixels have the same dwell
    int x0 = d_offs[2*blockIdx.x];
    int y0 = d_offs[2*blockIdx.y+1];
    ...
    if(comm_dwell != DIFF_DWELL) {
        ...
        if(x < d && y < d) {
            x += x0, y += y0;
            dwells[y * w + x] = comm_dwell;
        }
        val = 0;
    } else if(depth + 1 < MAX_DEPTH && d / SUBDIV > MIN_SIZE) {
        if (tid<32){
            d_offs[(blockIdx.y*gridDim.y + blockIdx.x)*SUBDIV*SUBDIV*2 + tid] = (y0+(d/SUBDIV)*(tid/2))*((tid)%2)+(x0+(d/SUBDIV)*(tid/2))*((tid+1)%2); 
        }
        val = 1;
    } else {
        int x = threadIdx.x + blockDim.x * blockIdx.x;
...
void mandelbrot_pseudo_dynamic_parallelism(int *dwell, int* h_nextSize, int* d_nextSize, int* d_offsets, int w, int h, complex cmin, complex cmax, int d, int depth){
    
	dim3 b(BSX, BSY, 1), g(*h_nextSize, INIT_SUBDIV, INIT_SUBDIV);
    border_dwell2<<<g, b>>>(d_nextSize, d_offsets, dwell, h, w, cmin, cmax, d, depth);
    ...
    if(*res != DIFF_DWELL) {
        ...
        dwell_fill_k<<<grid, b, 0, stream[i*g.x+j]>>>(dwell, w, x0, y0, d, *res);
    } else if(depth + 1 < MAX_DEPTH && d / SUBDIV > MIN_SIZE) {
        // subdivide recursively
        dim3 grid(SUBDIV, SUBDIV);
        mandelbrot_pseudo_dynamic_parallelism(dwell, w, h, cmin, cmax, newx, newy, d/SUBDIV, b, grid, depth+1);
    } else {
        // leaf, per-pixel kernel
        dim3 grid(divup(d, BSX), divup(d, BSY));
        mandelbrot_pixel_k<<<grid, b, 0, stream[i*g.x+j]>>>
...
int main(int argc, char **argv) {
    ...
    *h_nextSize = INIT_SUBDIV*INIT_SUBDIV;
	h_offsets = (int*)malloc(sizeof(int)*2*(INIT_SUBDIV*INIT_SUBDIV)*pow(SUBDIV, MAX_DEPTH-1));
    for (int i=0; i<INIT_SUBDIV*INIT_SUBDIV*2; i+=2){
        h_offsets[i] = (i%INIT_SUBDIV)*(W/INIT_SUBDIV);
        h_offsets[i+1] = (i/INIT_SUBDIV)*(W/INIT_SUBDIV);
    }
	cucheck(cudaMalloc((void**)&d_nextSize, sizeof(int)));
	cucheck(cudaMalloc((void**)&d_offsets, sizeof(int)*2*(INIT_SUBDIV*INIT_SUBDIV)*pow(SUBDIV, MAX_DEPTH-1)));
    cucheck(cudaMemcpy(d_offsets, h_offsets, sizeof(int)*2*(INIT_SUBDIV*INIT_SUBDIV)*pow(SUBDIV, MAX_DEPTH-1), cudaMemcpyHostToDevice))

	// compute the dwells, copy them back
	double t1 = omp_get_wtime();
	dim3 bs(BSX, BSY), grid(INIT_SUBDIV, INIT_SUBDIV);
    mandelbrot_pseudo_dynamic_parallelism(d_dwells, h_nextSize, d_nextSize, d_offsets, w, h, complex(-1.5, -1), complex(0.5, 1), 0, 0, W / INIT_SUBDIV, bs, grid, 1);
