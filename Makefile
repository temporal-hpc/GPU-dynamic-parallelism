...
build:
	g++ -O3 -fopenmp -lpng -D${VERBOSE} -D${MEASURE_POWER} -D${GRIDLINES} -D${DEBUG} -D${BENCHMARK} -DBSX=$(BSX) -DBSY=$(BSY) -DREALIZATIONS=${REALIZATIONS} -DREPEATS=$(REPEATS) -DDP_PENDING_KERNEL_BUFFER=${DP_PENDING_KERNEL_BUFFER} -c src/Rapl.cpp -o obj/Rapl.o
	...
	nvcc -O3 -arch=${ARCH} ${DP_RDC} -Xcompiler -fopenmp -lpng -DARCH=$(ARCH) -D${VERBOSE} -D${MEASURE_POWER} -D${GRIDLINES} -D${DEBUG} -D${BENCHMARK} -DBSX=$(BSX) -DBSY=$(BSY) -DREALIZATIONS=${REALIZATIONS} -DREPEATS=$(REPEATS) -DDP_PENDING_KERNEL_BUFFER=${DP_PENDING_KERNEL_BUFFER} obj/nvmlPower.o obj/Rapl.o obj/main.o -o $(TGT)
